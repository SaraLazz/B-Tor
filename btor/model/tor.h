...
{
  uint32_t size; 
  uint8_t data[CELL_NETWORK_SIZE]; 
};
...
{
public:
  Circuit (uint16_t, string, Ptr<Connection>, Ptr<Connection>, int, int);
  ~Circuit ();
  void DoDispose ();
...
// private:
  ...
  void PackageRelayCellImpl (uint16_t, Ptr<Packet>,uint8_t);

  void BridgeRelayCell (Ptr<Connection> conn, Ptr<Packet> cell,uint16_t id_circ_bridge); //NEW!!
